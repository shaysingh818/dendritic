...
mod decision_tree_tests {
    ...
    fn test_split() {
       ...
       vec![8, 1],
        ...
        assert_eq!(left.shape().values(), vec![2, 2]);
        assert_eq!(right.shape().values(), vec![6, 2]);

        let left_vals = left.axis(1, 0).unwrap();
        let right_vals = right.axis(1, 0).unwrap();

        assert_eq!(left_vals.values(), &vec![65.0, 65.0]);
        assert_eq!(
            right_vals.values(),
            &vec![69.0, 69.0, 72.0, 73.0, 70.0, 73.0]
        );

        assert_eq!(left_2.shape().values(), vec![4, 2]);
        assert_eq!(right_2.shape().values(), vec![4, 2]);
        ...
        assert_eq!(
            left_vals_2.values(), 
            &vec![4.39, 4.21, 4.09, 4.27]
        );
   
        assert_eq!(
            right_vals_2.values(), 
            &vec![5.85, 5.68, 5.56, 5.79]
        );
    ...
    fn test_information_gain() {
       ...
       vec![8, 1],
        ...
        assert_eq!(info_gain < info_gain_2, true);
