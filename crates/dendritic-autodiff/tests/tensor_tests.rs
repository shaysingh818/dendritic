...
mod value_test {
    ...
    fn test_value_generics() {
        ...
        let val_a: Tensor<f64> = Tensor::new(&a); 
        let val_b: Tensor<usize> = Tensor::new(&b); 
        let val_c: Tensor<i64> = Tensor::new(&c); 
        let val_d: Tensor<&str> = Tensor::new(&char_val); 
        let val_e = Tensor::new(&array_val);
