...
pub struct ElasticRegressionSerialize {
...
impl ElasticRegression {

    pub fn new(
        x: &Array2<f64>,
        y: &Array2<f64>,
        learning_rate: f64,
        lambda: f64) -> Result<RidgeRegression, String> {
...
impl DescentOptimizer for ElasticRegression {
    ...
    fn parameter_update(&mut self) {
        ...
        if let Some(outputs) = self.outputs.as_ref() {
            ...
            let sig_w = w.clone().output().mapv(|x| x.signum());
            let l2_w_grad = w.grad() + self.lambda * 2.0 * w.output();
            let l1_w_grad = w.grad() + (self.lambda * sig_w);

            let w_grad = l2_w_grad + l1_w_grad;
            let w_delta = w.output() - self.learning_rate * w_grad;
    ...
    fn train(&mut self, epochs: usize) {
        ...
        let l2_penalty = self.weights.mapv(|x| (x as f64).powf(2.0)).sum();

        let loss_node = self.graph.curr_node();
        let loss = loss_node.output();
        let total_loss = loss.clone() + (self.lambda * l2_penalty);
        println!(
    ...
    fn save(&self, filepath: &str) -> std::io::Result<()> {
        ...
        let obj = RidgeRegressionSerialize {
            graph_path: format!("{filepath}/ridge_regression_exp"),
    ...
    fn save_snapshot(&self, namespace: &str) -> std::io::Result<()> {
        ...
        let obj = RidgeRegressionSerialize {
            graph_path: format!("{namespace}/ridge_regression_exp"),
            weights: self.weights.clone(), 
            bias: self.bias.clone(),
            learning_rate: self.learning_rate,
            lambda: self.lambda
